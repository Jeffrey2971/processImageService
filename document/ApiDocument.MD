...
| ------------------ | -------- | -------- | ------------------------------------------------------------ |
| X-JEFFREY-API-SIGN | 是       | String   | 1. 签名规则为 **md5(publicKey + md5(image) + salt + privateSecret)** <br />2. md5(image) 中的 image 是原始图片数据，不加任何编码，md5 校验过后为32位小写<br />3. salt 为随机数，由 8 位随机 int 类型的数字组合而成，注意生成随机数的第一个字符不要为 0 ，否则造成签名失败，例如 `01234567`<br />3. publicKey 为你的公钥，由数字、大小写字母随机组合而成<br />4. privateSecret 为你的密钥，由 128 位数字、大小写字母、特殊字符组合而成，**确保不在请求参数中携带该参数，仅在生成签名时使用**<br />5. 最终的 sign 为 32 位小写字符串 |
...
<img src="/Users/jeffrey/Documents/JeffreyNoteRemoteData/images/wx.JPG" alt="微信打赏" style="zoom: 25%;" />



<img src="/Users/jeffrey/Documents/JeffreyNoteRemoteData/images/zfb.JPG" alt="支付宝打赏" style="zoom:25%;" />