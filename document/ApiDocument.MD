...
| ----------------- | -------- | -------- | ------------------------------------------------------------ |
| publicKey         | 是       | String   | publicKey 为你的公钥，由数字、大小写字母随机组合而成，总长度 16 位，通过注册账号获取 |
| salt              | 是       | Integer  | salt 为盐值，由 8 位随机 int 类型的数字组合而成，注意生成随机数的第一位不要为 0 ，否则造成签名失败，例如 `01234567` |
| x-access-api-sign | 是       | String   | 1. 该参数为头部信息，请在请求头中指定该参数<br />2. 签名规则为 **md5(publicKey + imageMd5 + salt + privateKey)** <br />3. imageMd5 为请求图片的 md5 信息，不加任何编码，将图片转换为字节数组后进行计算，md5 校验计算完成后为 32 位小写<br />4. privateKey 为你的私钥，由数字、大小写字母、特殊字符组合而成，总长度 32 位，**请注意，确保在请求参数以及网络传输过程中不携带该参数，仅在客户端生成签名时使用**<br />5. 最终的 sign 为 32 位小写字符串 |
