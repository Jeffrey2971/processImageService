...
- 图片去水印 API 请求地址
  - https://www.processimage.cn:4433/access
...
- 在接入前首先需要进行接口安全校验，支持两种校验方式，分别为签名方式和 token 鉴权方式，前者安全性较高但效率相对于后者略低，后者效率较高但安全性较低，在开始接入前，请先 [点击注册账号](https://www.processimage.cn:4433/user/main) 并获取属于您的 api 公钥和私钥或配置您的 IP 白名单，以便在后续可参与安全校验
...
| rectangles      | Json    | 否                                                     | 无    | 矩形坐标，为空默认将识别到图中的文字水印信息作为矩形坐标，如提供该参数，请正确填写，如提供有误则请求不成功，明确指定该参数后 `watermarkNames / excludeKeywords / offset` 将无效且服务器不会识别出图片中的文字矩形区域，可提供多个矩形坐标，例如：<br />[<br/>      {<br/>        "x": 120,<br/>        "y": 76,<br/>        "w": 100,<br/>        "h": 173<br/>      },<br/>      {<br/>        "x": 114,<br/>        "y": 80,<br/>        "w": 132,<br/>        "h": 206<br/>      }<br/>    ] | 可用                                 |
| watermarkNames  | Json    | 否                                                     | 无    | 根据给定的水印名称去除识别到的图中的文字水印名称，该参数仅在不明确指定矩形坐标参数时生效，且该参数和 excludeKeywords 只能指定其中一个生效，参数可提供多个，例如：{<br/>  "watermarkNames": [<br/>    "名称A",<br/>    "名称B"<br/>  ]<br/>} | 完成，试验型功能，在使用上会进行优化 |
| excludeKeywords | Json    | 否                                                     | 无    | 排除图片中的水印文字，该功能仅在不明确指定矩形坐标参数时生效，且该参数和 markNames 只能指定其中一个，参数可提供多个，例如：<br />{<br/>  "keywords": [<br/>    "小红书",<br/>    "id：10086"<br/>  ]<br/>} | 完成，试验型功能，在使用上会进行优化 |
| offset          | Json    | 否                                                     | 无    | 矩形偏移量，在识别到的文本矩形框**额外**缩小或放大矩形大小，仅能使用 `+x + y +w +h -x -y -w -h` 指定缩小还是放大偏移量，如识别出的文字矩形区域额外添加偏移量后造成矩形超出图像框架范围，则忽略 offset 参数重试并响应。例子：{<br/>  "+x": 5,<br/>  "-y": 8,<br/>  "-w": 3,<br/>  "+h": 4<br/>} | 完成                                 |
...
- 响应码分为 HTTP 响应码以及 PROCESS 处理状态码，HTTP 响应码反映了本次调用是否成功，校验是基于用户是否在黑名单内、访问频率校验、账号可使用次数校验、请求以及请求参数是否合法校验，而 PROCESS 处理状态码是更偏向于本次图片处理是否成功，例如处理过程中是否发生异常、根据用户指定的参数导致处理图片的不同结果的响应。总的来说，HTTP 响应码更偏向于本次是否能请求，而 PROCESS 处理状态码更偏向于本次请求的图片是否完成
...
```json
{
  ...
  "data": {
    ...
    "url": "https://www.processimage:4433.cn/targetImages/1669620124079.jpeg"
  }
}
```
...
- 以下为调用缓存结果返回数据的例子
  ...
  ```json
  {
    ...
    "data": {
      ...
      "url": "https://www.processimage:4433.cn/targetImages/1670915997800.jpeg"
    },
    "encryptedInfo": {
      "id": 1,
...
- 啥也不说了各位看官老板，项目发展维护弄啥都要钱

<img src="https://www.processimage.cn:4433/img/wx.jpg" alt="微信打赏" style="zoom: 25%;" />

<img src="https://www.processimage.cn:4433/img/zfb.jpg" alt="支付宝打赏" style="zoom: 25%;" />
